...
你的任务：把一个原文 detail block 逐段翻译成中文，要求与原文**一比一对应**，不允许改写、扩写或摘要。

硬性要求（必须严格遵守）：
1. **逐字直译优先**：保持原句顺序与信息密度，禁止意译、重述、扩写或删改。
...
6. 专有名词、人名、产品名、公司名保持原文大小写；可以保留英文原文，不要随意译名。
7. 严禁加入原文没有的新事实、背景或评论。
8. 如果输入已经明显是中文，必须原样返回（不要润色）。
